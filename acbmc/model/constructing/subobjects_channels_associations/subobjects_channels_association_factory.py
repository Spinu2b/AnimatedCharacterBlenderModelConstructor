...
from acbmc.model.model.subobjects_channels_associations_desc.subobjects_channels_association import \
...
class SubobjectsChannelsAssociationDescriptionFactory:
    def construct_from_json_dict(self, subobjects_channels_association_description_json_dict) -> SubobjectsChannelsAssociationDescription:
        ...
        result.channels_for_subobjects_parenting = self._get_channels_for_subobjects_parenting(
            subobjects_channels_association_description_json_dict["channelsForSubobjectsParenting"])
        result.channels_for_subobjects_bones_parenting = self._get_channels_for_subobjects_bones_parenting(
            subobjects_channels_association_description_json_dict["channelsForSubobjectsBonesParenting"]
        )
        return result

    def _get_channels_for_subobjects_parenting(self, channels_for_subobjects_parenting_json_dict) -> Dict[int, List[int]]:
        return {key:[x for x in value_list] for (key,value_list) in channels_for_subobjects_parenting_json_dict.items()}
    ...
    def _get_channels_for_subobjects_bones_parenting(self, channels_for_subobjects_bones_parenting_json_dict) -> Dict[int, Dict[int, List[int]]]:
        return {outer_key:{inner_key:[x for x in inner_list] for (inner_key,inner_list) in outer_value.items()} for (outer_key,outer_value)
