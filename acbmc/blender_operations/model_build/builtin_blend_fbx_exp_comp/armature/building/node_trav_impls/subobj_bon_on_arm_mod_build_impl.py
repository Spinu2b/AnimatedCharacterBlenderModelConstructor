...
from acbmc.blender_operations.model_build \
    .builtin_blend_fbx_exp_comp.armature.building \
    .node_trav_impls.node_trav_arm_build_impl import NodeTraversalArmatureBuildingImplementation


class SubobjectsBonesOnlyArmatureModelBuilderImplementation(NodeTraversalArmatureBuildingImplementation):
    def _complete_model_with_proper_deform_set_bones(
        ...
        if subobject.contains_actual_bones():
            for bone_index, bone_bind_pose in subobject.geometric_object.bind_bone_poses.items():
                ...
                bone_node = self.blender_armature_model_factory.get_appropriate_subobject_actual_bone_node(bone_bind_pose, bone_name)
            ...
            bone_node = self.blender_armature_model_factory.get_appropriate_subobject_parent_bone_node(bone_name)
            # bone_node = EditModeBoneNode()
            # bone_node.bone_name = bone_name
    ...
    def get_blender_armature_model(self, data: any) -> TreeHierarchy:
        ...
        root_bone_node = self.blender_armature_model_factory.get_appropriate_subobject_parent_bone_node(root_bone_name)
